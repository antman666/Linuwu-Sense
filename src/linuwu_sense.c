...
static int __init
dmi_matched (const struct dmi_system_id *dmi)
{
...
static int __init
set_force_caps (const struct dmi_system_id *dmi)
{
    if (force_caps == -1)
        {
...
static acpi_status
wmab_execute (struct wmab_args *regbuf, struct acpi_buffer *result)
{
...
static acpi_status
AMW0_get_u32 (u32 *value, u32 cap)
{
    int err;
    u8 result;
...
static acpi_status
AMW0_set_u32 (u32 value, u32 cap)
{
...
static acpi_status
WMID_get_u32 (u32 *value, u32 cap)
{
    acpi_status status;
    u8 tmp;
...
static acpi_status
WMID_set_u32 (u32 value, u32 cap)
{
    u32 method_id = 0;
    char param;
...
static acpi_status
wmid3_get_device_status (u32 *value, u16 device)
{
    ...
    struct acpi_buffer input
        = { sizeof (struct wmid3_gds_get_input_param), &params };
...
static acpi_status
wmid_v2_get_u32 (u32 *value, u32 cap)
{
    u16 device;
...
static acpi_status
wmid3_set_device_status (u32 value, u16 device)
{
    struct wmid3_gds_return_value return_value;
    acpi_status status;
    union acpi_object *obj;
    ...
    struct acpi_buffer get_input
        = { sizeof (struct wmid3_gds_get_input_param), &get_params };
    ...
    struct acpi_buffer set_input
        = { sizeof (struct wmid3_gds_set_input_param), &set_params };
    struct acpi_buffer output = { ACPI_ALLOCATE_BUFFER, NULL };
...
static acpi_status
wmid_v2_set_u32 (u32 value, u32 cap)
{
    u16 device;
...
static void __init
type_aa_dmi_decode (const struct dmi_header *header, void *d)
{
...
static int
WMI_gaming_execute_u32_u64 (u32 method_id, u32 in, u64 *out)
{
    struct acpi_buffer result = { ACPI_ALLOCATE_BUFFER, NULL };
    struct acpi_buffer input = {
        .length = sizeof (in),
...
static acpi_status
WMID_gaming_set_u64 (u64 value, u32 cap)
{
    u32 method_id = 0;
...
static acpi_status
WMID_gaming_get_u64 (u64 *value, u32 cap)
{
    acpi_status status;
    u64 result;
    u64 input;
...
static int
WMID_gaming_get_sys_info (u32 command, u64 *out)
{
    acpi_status status;
    u64 result;
    ...
    if (interface->capability & cap)
        {
            switch (interface->type)
                ...
                case ACER_WMID_v2:
                    if (cap
                        & (ACER_CAP_WIRELESS | ACER_CAP_BLUETOOTH
                           | ACER_CAP_THREEG))
 ...
 */
static void
mail_led_set (struct led_classdev *led_cdev, enum led_brightness value)
{
...
static int
acer_led_init (struct device *dev)
{
...
static int
update_bl_status (struct backlight_device *bd)
{
...
static int
acer_backlight_init (struct device *dev)
{
...
static int
acer_gsensor_open (struct input_dev *input)
{
    return acer_gsensor_init ();
}

static int
acer_gsensor_event (void)
{
    acpi_status status;
    struct acpi_buffer output;
...
acer_predator_v4_platform_profile_probe (void *drvdata, unsigned long *choices)
{
    ...
    err = WMID_gaming_get_misc_setting (
        ACER_WMID_MISC_SETTING_SUPPORTED_PROFILES, (u8 *)&supported_profiles);
    if (err)
        return err;
    ...
    if (test_bit (ACER_PREDATOR_V4_THERMAL_PROFILE_TURBO, &supported_profiles))
        {
            ...
            /* We need to handle the hypothetical case where only the turbo
             * profile is supported. In this case the turbo toggle will
...
static int
acer_platform_profile_setup (struct platform_device *pdev)
{
...
static int
acer_thermal_profile_change (void)
{
    /*
 ...
 */
static int
acer_kbd_dock_state_to_sw_tablet_mode (u8 kbd_dock_state)
{
...
static void
acer_kbd_dock_get_initial_state (void)
{
    u8 *output, input[8] = {
        0x05,
        0x00,
...
static void
acer_kbd_dock_event (const struct event_return_value *event)
{
    int sw_tablet_mode;
...
static void
acer_rfkill_update (struct work_struct *ignored)
{
    u32 state;
    acpi_status status;
...
static int
acer_rfkill_set (void *data, bool blocked)
{
    acpi_status status;
...
static struct rfkill *
acer_rfkill_register (struct device *dev, enum rfkill_type type, char *name,
                      u32 cap)
{
    int err;
...
static int
acer_rfkill_init (struct device *dev)
{
    int err;

    if (has_cap (ACER_CAP_WIRELESS))
        {
            wireless_rfkill = acer_rfkill_register (
                dev, RFKILL_TYPE_WLAN, "acer-wireless", ACER_CAP_WIRELESS);
    ...
    if (has_cap (ACER_CAP_BLUETOOTH))
        {
            bluetooth_rfkill
                = acer_rfkill_register (dev, RFKILL_TYPE_BLUETOOTH,
    ...
    if (has_cap (ACER_CAP_THREEG))
        {
            threeg_rfkill = acer_rfkill_register (
                dev, RFKILL_TYPE_WWAN, "acer-threeg", ACER_CAP_THREEG);
...
static void
acer_wmi_notify (union acpi_object *obj, void *context)
{
    ...
    switch (return_value.function)
        ...
        case WMID_HOTKEY_EVENT:
            ...
            if (!key)
            ...
            else
                {
                    ...
                    sparse_keymap_report_event (acer_wmi_input_dev, scancode,
                                                1, true);
                }
            break;
        ...
        case WMID_CALIBRATION_EVENT:
            if (has_cap (ACER_CAP_PREDATOR_SENSE)
                || has_cap (ACER_CAP_NITRO_SENSE)
                || has_cap (ACER_CAP_NITRO_SENSE_V4))
                {
                    if (battery_health_set (CALIBRATION_MODE,
                                            return_value.key_num)
                        != AE_OK)
                        {
 ...
 */
static ssize_t
predator_usb_charging_show (struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    acpi_status status;
    u64 result;
...
static ssize_t
predator_usb_charging_store (struct device *dev, struct device_attribute *attr,
...
static acpi_status
battery_health_query (int mode, int *enabled)
{
...
static acpi_status
battery_health_set (u8 function, u8 function_status)
{
    ...
    status = wmi_evaluate_method (
        WMID_GUID5, 0, ACER_WMID_SET_BATTERY_HEALTH_CONTROL_METHODID, &input,
        &output);
    if (ACPI_FAILURE (status))
        return status;
...
static ssize_t
predator_battery_limit_show (struct device *dev, struct device_attribute *attr,
                             char *buf)
{
...
static ssize_t
predator_battery_limit_store (struct device *dev,
...
static ssize_t
predator_battery_calibration_show (struct device *dev,
...
static ssize_t
preadtor_battery_calibration_store (struct device *dev,
...
static u64
fan_val_calc (int percentage, int fan_index)
{
    return (((percentage * 25600) / 100) & 0xFF00) + fan_index;
}
static acpi_status
acer_set_fan_speed (int t_cpu_fan_speed, int t_gpu_fan_speed)
{
    ...
    if (t_cpu_fan_speed == 100 && t_gpu_fan_speed == 100)
        {
            ...
            status = WMI_gaming_execute_u64 (
                ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x820009, NULL);
            if (ACPI_FAILURE (status))
                {
    ...
    else if (t_cpu_fan_speed == 0 && t_gpu_fan_speed == 0)
        {
            ...
            status = WMI_gaming_execute_u64 (
                ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x410009, NULL);
            if (ACPI_FAILURE (status))
                {
                    pr_err ("Error setting fan speed status: %s\n",
                            acpi_format_exception (status));
            ...
            if (t_cpu_fan_speed == 0)
                {
                    ...
                    status = WMI_gaming_execute_u64 (
                        ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x10001,
                        NULL);
                    if (ACPI_FAILURE (status))
                        {
                            pr_err ("Error setting fan speed status: %s\n",
                    ...
                    status = WMI_gaming_execute_u64 (
                        ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0xC00008,
                        NULL);
                    if (ACPI_FAILURE (status))
                        {
                            pr_err ("Error setting fan speed status: %s\n",
                    ...
                    status = WMI_gaming_execute_u64 (
                        ACER_WMID_SET_GAMING_FAN_SPEED_METHODID,
            ...
            else if (t_gpu_fan_speed == 0)
                {
                    ...
                    status = WMI_gaming_execute_u64 (
                        ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x400008,
                        NULL);
                    if (ACPI_FAILURE (status))
                        {
                            pr_err ("Error setting fan speed status: %s\n",
                    ...
                    status = WMI_gaming_execute_u64 (
                        ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x30001,
                        NULL);
                    if (ACPI_FAILURE (status))
                        {
                            pr_err ("Error setting fan speed status: %s\n",
            ...
            else
                {
                    ...
                    status = WMI_gaming_execute_u64 (
                        ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0xC30009,
                        NULL);
                    if (ACPI_FAILURE (status))
                        {
                            pr_err ("Error setting fan speed status: %s\n",
...
static ssize_t
predator_fan_speed_store (struct device *dev, struct device_attribute *attr,
                          const char *buf, size_t count)
{
...
static int
acer_predator_state_update (int value)
{
    u8 current_tp;
    int tp, err;
    err = WMID_gaming_get_misc_setting (
...
acer_predator_state_restore (int value)
{
    ...
    acpi_status status = acer_set_fan_speed (
        value == 0 ? current_states.battery_state.cpu_fan_speed
 ...
 */
static ssize_t
predator_lcd_override_show (struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    acpi_status status;
    u64 result;
    status = WMI_gaming_execute_u64 (ACER_WMID_GET_GAMING_PROFILE_METHODID,
                                     0x00, &result);
    if (ACPI_FAILURE (status))
        {
...
static ssize_t
predator_lcd_override_store (struct device *dev, struct device_attribute *attr,
                             const char *buf, size_t count)
{
    acpi_status status;
    u64 result;
    ...
    status
        = WMI_gaming_execute_u64 (ACER_WMID_SET_GAMING_PROFILE_METHODID,
...
static ssize_t
predator_backlight_timeout_show (struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
    acpi_status status;
    u64 result;
    status = WMI_apgeaction_execute_u64 (ACER_WMID_GET_FUNCTION, 0x88401,
                                         &result);
    if (ACPI_FAILURE (status))
        {
...
static ssize_t
predator_backlight_timeout_store (struct device *dev,
                                  struct device_attribute *attr,
                                  const char *buf, size_t count)
{
    acpi_status status;
 ...
 */
static ssize_t
predator_boot_animation_sound_show (struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    acpi_status status;
    u64 result;
    status = WMI_gaming_execute_u64 (
        ACER_WMID_GET_GAMING_MISC_SETTING_METHODID, 0x6, &result);
    if (ACPI_FAILURE (status))
        {
                                     ...
                                     const char *buf, size_t count)
{
    ...
    status
        = WMI_gaming_execute_u64 (ACER_WMID_SET_GAMING_MISC_SETTING_METHODID,
...
static acpi_status
set_kb_status (int mode, int speed, int brightness, int direction, int red,
               int green, int blue)
{
    u64 resp = 0;
    u8 gmInput[16] = { mode, speed, brightness, 0, direction, red, green, blue,
                       3,    1,     0,          0, 0,         0,   0,     0 };
    ...
    struct acpi_buffer output = { ACPI_ALLOCATE_BUFFER, NULL };
    struct acpi_buffer input
        = { (acpi_size)sizeof (gmInput), (void *)(gmInput) };
...
static acpi_status
get_kb_status (struct get_four_zoned_kb_output *out)
{
    u64 in = 1;
    acpi_status status;
    union acpi_object *obj;
...
/* four zone mode */
static ssize_t
four_zoned_rgb_kb_show (struct device *dev, struct device_attribute *attr,
                        char *buf)
{
    acpi_status status;
...
static ssize_t
four_zoned_rgb_kb_store (struct device *dev, struct device_attribute *attr,
                         const char *buf, size_t count)
{
    acpi_status status;
...
static acpi_status
get_per_zone_color (struct per_zone_color *output)
{
    acpi_status status;
    u64 *zones[]
    ...
    for (int i = 0; i < 4; i++)
        {
            status = WMI_gaming_execute_u64 (
                ACER_WMID_GET_GAMING_RGB_KB_METHODID, zone_ids[i], zones[i]);
            if (ACPI_FAILURE (status))
                {
...
static acpi_status
set_per_zone_color (struct per_zone_color *input)
{
    acpi_status status;
    u64 *zones[]
        = { &input->zone1, &input->zone2, &input->zone3, &input->zone4 };
    u8 zone_ids[] = { 0x1, 0x2, 0x4, 0x8 };
    ...
    for (int i = 0; i < 4; i++)
        {
            *zones[i] = (cpu_to_be64 (*zones[i]) >> 32) | zone_ids[i];
            status = WMI_gaming_execute_u64 (
                ACER_WMID_SET_GAMING_RGB_KB_METHODID, *zones[i], NULL);
            if (ACPI_FAILURE (status))
                {
...
static ssize_t
per_zoned_rgb_kb_show (struct device *dev, struct device_attribute *attr,
                       char *buf)
{
...
static ssize_t
per_zoned_rgb_kb_store (struct device *dev, struct device_attribute *attr,
                        const char *buf, size_t count)
{
    int i = 0;
    ssize_t len;
...
static int
four_zone_kb_state_update (void)
{
    acpi_status status;
    struct get_four_zoned_kb_output out;
...
static void
acer_platform_shutdown (struct platform_device *device)
{
    struct acer_data *data = &interface->data;
...
static umode_t
acer_wmi_hwmon_is_visible (const void *data, enum hwmon_sensor_types type,
...
static int
acer_wmi_hwmon_read (struct device *dev, enum hwmon_sensor_types type,
...
acer_wmi_hwmon_init (void)
{
    ...
    hwmon = devm_hwmon_device_register_with_info (
        dev, "acer", &supported_sensors, &acer_wmi_hwmon_chip_info, NULL);
