			      ...
			      enum linuwu_sense_fan_mode *fan_mode)
{
	...
	case LINUWU_SENSE_FAN_CPU:
		input_value = FIELD_PREP(ACER_GAMING_FAN_BEHAVIOR_ID_MASK,
					 ACER_GAMING_FAN_BEHAVIOR_CPU);
		break;
	...
	input.length = sizeof(input_value);
	input.data = &input_value;

	err = wmidev_invoke_method(wdev, 0,
				   ACER_WMID_GET_GAMING_FAN_BEHAVIOR_METHODID,
				   &input, &output, sizeof(result));
	if (err)
		return err;
			       ...
			       int percentage)
{
	...
	input_value |= FIELD_PREP(ACER_GAMING_FAN_SPEED_ID_MASK, fan_id);
	input_value |= FIELD_PREP(ACER_GAMING_FAN_SPEED_VALUE_MASK, percentage);
	input.length = sizeof(input_value);
	input.data = &input_value;
